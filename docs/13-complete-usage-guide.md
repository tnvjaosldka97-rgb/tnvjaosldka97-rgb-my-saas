...
이 문서는 my-saas를 처음 접하는 개발자가 로컬 세팅부터 프로덕션 배포, 커스터마이징까지 모든 것을 순서대로 따라할 수 있도록 작성되었습니다.

---

## 목차

1. [my-saas란?](#1-my-saas란)
2. [사전 준비물](#2-사전-준비물)
...
my-saas는 **Cloudflare Workers 기반 풀스택 SaaS 보일러플레이트**입니다. 하나의 에지 런타임에서 프론트엔드, API, 데이터베이스, AI, 미디어, 이메일, CMS를 모두 운영할 수 있습니다.
...
```text
Token name:     my-saas-deploy (원하는 이름)
...
my-saas는 **pnpm workspace 모노레포**입니다. 하나의 저장소에 4개 패키지가 있습니다:

| 패키지 | 경로 | 역할 |
|--------|------|------|
| `@my-saas/worker` | `worker/` | API 서버 + 정적자산 서빙 |
| `@my-saas/landing` | `apps/landing/` | 공개 마케팅 페이지 |
| `@my-saas/admin` | `apps/admin/` | 인증 어드민 콘솔 |
| `@my-saas/com` | `packages/com/` | 공유 타입 (contracts.ts) |
...
요청 → enforceAdminAccess 미들웨어
  ...
  │   └── __Host-my-saas_admin 쿠키 → JWT 검증 → 이메일 확인
...
성공 시:
- `__Host-my-saas_admin` 쿠키 발급 (JWT, 12시간 만료)
...
my-saas에는 5개 샘플 페이지가 포함되어 있습니다:
...
```bash
pnpm test                          # 전체
pnpm --filter @my-saas/worker test  # Worker만
```
...
```bash
pnpm wrangler vectorize create my-saas-doc-index \
  --dimensions 384 \
  --metric cosine
```
...
```bash
pnpm wrangler r2 bucket create my-saas-media
...
my-saas를 복사해서 새 프로젝트를 시작할 때:

### 1. package.json 교체 (5곳)

```bash
# 루트
package.json: "my-saas" → "my-saas"

# Worker
worker/package.json: "@my-saas/worker" → "@my-saas/worker"

# Apps
apps/landing/package.json: "@my-saas/landing" → "@my-saas/landing"
apps/admin/package.json: "@my-saas/admin" → "@my-saas/admin"

# 공유 패키지
packages/com/package.json: "@my-saas/com" → "@my-saas/com"
```
...
| D1 쿼리 에러 | 마이그레이션 미적용 | `pnpm --filter @my-saas/worker db:migrate:local` |
